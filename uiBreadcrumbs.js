 ...
 *
 * Modified by Ilya Savinkov - additional breadcrumbs feature:
 * You can specify a setting "breadcrumb-property" and say in which state property an array of additional breadcrumbs is stored,
 * just like "displayname-property". Then you can fill the breadcrumbs array with additional objects which have two keys: displayName (required) and route (optional)
 */
    ...
    module.directive('uiBreadcrumbs', ['$interpolate', '$state', function($interpolate, $state) {
        return {
            ...
            link: function(scope) {
                ...
                scope.$on('$updateBreadcrumbs', function() {
                    updateBreadcrumbsArray();
                });
                        ...
                        if (workingState) {
                            ...
                            if (displayName !== false && !stateAlreadyInBreadcrumbs(workingState, breadcrumbs)) {
                                breadcrumbs.push({
                                    ...
                                    route: workingState.abstract ? null : workingState.name
                                });
                            }
                        }

                        if(additionalBreadcrumbs = getAdditionalBreadcrumbs(workingState)) {
                            breadcrumbs = breadcrumbs.concat(angular.copy(additionalBreadcrumbs).reverse());
                        }
                ...
                function getWorkingState(currentState) {
                    ...
                    if (currentState.abstract === true) {
                        ...
                        } else if(!currentState.displayAbstractBreadcrumb) {
                            workingState = false;
                        }
                    }
                    return workingState;
                 ...
                 * @returns {*}
                 */
                function getAdditionalBreadcrumbs(currentState) {
                    if (!scope.breadcrumbsProperty) {
                        return false;
                    }
                    var breadcrumbs = getObjectValue(scope.breadcrumbsProperty, currentState),
                        parentBreadcrumbs = currentState.parent ? getObjectValue(scope.breadcrumbsProperty, currentState.parent) : null;

                    //we need breadcrumbs only if it was defined in the current state but not inherited from parent
                    if(!breadcrumbs || parentBreadcrumbs == breadcrumbs) {
                        return false;
                    }
                    ...
                    //deep copy collection into a new variable
                    angular.forEach(breadcrumbs, function(crumb) {
                        this.push(angular.extend({}, crumb));
                    }, crumbsCopy);

                    for(var i = 0; i < crumbsCopy.length; ++i) {
                        crumbsCopy[i].displayName = $interpolate(crumbsCopy[i].displayName)(interpolationContext);
                        if(!crumbsCopy[i].displayName) {
                            crumbsCopy.splice(i, 1);
                            --i;
                        }
                    }
